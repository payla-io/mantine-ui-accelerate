...
interface FormFooterProps {
    ...
    formInstance: UseFormReturnType<Record<string, unknown>,(values: Record<string, unknown>) => Record<string, unknown>>
    isPending?: boolean
    backLabel?: string
    submitLabel?: string
...
export default function FormFooter(props: Readonly<FormFooterProps>) {
    ...
    {props.singleQuestion && props.currentIndex > 0 && !props.disableBack && <Button
      onClick={() => {
        props.handleBack()
        setButtonType('button')
      }}
    ...
    <Flex justify={'right'} {...props.submitButtonContainerProps}>
      {props.singleQuestion && !props.data.inputProps.required && <Button variant='subtle' onClick={() => {
          props.handleContinue()
        }} mr="sm" {...props.skipButtonProps}>{props.backLabel ?? "Skip"}</Button>}
      <Button
        onClick={() => {
      ...
      >
        {(!props.singleQuestion || (lastQuestion))  ? (props.submitLabel ?? "Save") : (props.continueLabel ?? "Continue")}
      </Button>
    </Flex>
  </Flex>
  )
