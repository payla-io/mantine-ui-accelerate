...
export const isValidHttpUrl = (value: string) => {
    ...
    try {
      return (
        isValidUrl(value) || new URL(value).protocol === 'http:' || new URL(value).protocol === 'https:'
      )
    } catch (_) {
      return false
    }
...
export const conditionValidator: {[key: string]: any} = {
    ...
    is_character_length_less_than_or_equal_to : (currentValue: any, conditionValue: any) => {
      return currentValue?.toString().length <= conditionValue
    },
  
    is_word_length_greater_than_or_equal_to : (currentValue: any, conditionValue: any) => {
      return currentValue?.split(' ').length >= conditionValue
    },
  
    is_word_length_less_than_or_equal_to : (currentValue: any, conditionValue: any) => {
      return currentValue?.split(' ').length <= conditionValue
    },
    ...
    options_length_greater_than_or_equal_to : (currentValue: any, conditionValue: any) => {
      return currentValue?.length >= conditionValue
    },
    ...
    options_length_less_than_or_equal_to : (currentValue: any, conditionValue: any) => {
      return currentValue?.length <= conditionValue
    },
...
const buildErrorText = (comparison: string, conditionValue: any, conditionOptions: any) => {
...
export const validateConditions = (
    ...
    conditions.forEach((c) => {
      ...
      if (method) {
        ...
        const conditionOptions = c.options?.map(
          (o) => o.value,
        )
  
        const conditionOptionsTitle = c.options?.map(
          (o) => o.label,
        )
        const conditionValue = c.value
        const conditionWithOptions = ["options_in", "options_not_in"].includes(c.method)
        ...
        if (!currentResult) {
            failed.push(c.failedMessage ?? buildErrorText(c.method, conditionValue, conditionOptionsTitle))
        }
        if (result === null) {
